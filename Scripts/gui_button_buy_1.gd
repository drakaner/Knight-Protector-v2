...
@onready var child_node = gui_button_buy.get_node("hud_button_9000")
...
func _process(delta: float) -> void:
	...
	#print("toto : ", DataSave.nbTotalItemsEquipements)
	if Input.is_action_just_released("button_left"):
		child_node.modulate = Color(1, 1, 1, 0)

func _on_hud_button_9000_gui_input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("button_left"):
		#DataSave.hero.gold = 9000
		child_node.modulate = Color(1, 1, 1, 1)
		print("bon la ca marche pour le button buy 9000 !!!!!")
		if DataSave.hero.gold >= 9000 and DataSave.nbTotalItemsEquipements < 14:
			DataSave.hero.gold -= 9000
