...
@onready var child_button = gui_button_buy.get_node("hud_button_350")
...
func _on_hud_button_350_gui_input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("button_left"):
		child_button.modulate = Color(1, 1, 1, 1)
		if DataSave.hero.gold >= 350 and DataSave.nbTotalItemsEquipements < 14:
			DataSave.hero.gold -= 350
