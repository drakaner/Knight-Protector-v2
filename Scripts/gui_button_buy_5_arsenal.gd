...
@onready var child_button = gui_button_buy.get_node("hud_button_75")
...
func _on_hud_button_75_gui_input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("button_left"):
		child_button.modulate = Color(1, 1, 1, 1)
		if DataSave.hero.gold >= 75 and DataSave.nbTotalItemsEquipements < 14:
			DataSave.hero.gold -= 75
